...
\begin{equation}
    ...
    \end{cases}
\end{equation}
由于 $\displaystyle\lim_{x\to 0}x\sin \frac{1}{x}=0=f(0)$，故$f$在$x=0$处连续，但连续不一定可导，下面证明$f$在$x=0$处不可导，
...
\begin{table}[H]
    ...
    \caption{符号说明表}
    \label{符号说明表}
    \begin{tabularx}{0.7\textwidth}{|>{\centering\arraybackslash}X|c|>{\centering\arraybackslash}X|}
        \hline
        符号 & 含义 & 单位 \\
        \hline
        $E$ & 能量 & 焦耳 \\
